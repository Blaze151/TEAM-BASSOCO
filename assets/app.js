...
(function () {
  ...
  const STR = {
    es: {
      ...
      folio_label: "Folio",
      tiempo_entrega_cliente_label: "Tiempo de entrega aproximado",
    ...
    en: {
      ...
      unidad_prefix: "Unit",
      folio_label: "Folio",
      tiempo_entrega_cliente_label: "Approximate delivery time",
  ...
  const REPO_OWNER = "Blaze151", REPO_NAME = "TEAM-BASSOCO", REPO_BRANCH = "main";
  ...
  function ESTADOS_SERVICIO() {
    return {
      ...
      diagnostico: { label: t("estado_diagnostico_label"), corto: t("estado_diagnostico_corto"), icon: "🔍" },
  ...
  function renderActiveServiceCard(activo) {
    ...
    return `
      <div class="card" style="box-shadow: var(--neu-out), 0 0 0 1px rgba(111,190,68,0.35);">
        ...
        ${activo.tiempo_entrega_aprox ? `<div class="field" style="margin-top:12px;padding:10px 12px;background:rgba(232,201,74,0.08);border:1px solid rgba(232,201,74,0.3);border-radius:10px;"><div class="field-label" style="color:var(--yellow);">🕓 ${esc(t("tiempo_entrega_cliente_label"))}</div><div class="field-value">${esc(activo.tiempo_entrega_aprox)}</div></div>` : ""}
    ...
    if (servicios.length) {
      timelineHtml = `<ul class="timeline">` + servicios.map((s, i) => `
        <li class="timeline-item ${i === 0 ? "latest" : ""}">
          ...
          <div class="timeline-title">${esc(s.tipo)}${s.folio ? ` <span style="color:var(--green-bright);font-weight:400;font-size:12px;">· ${esc(t("folio_label"))} ${esc(s.folio)}</span>` : ""}${i === 0 ? `<span class="badge-latest">${esc(t("mas_reciente"))}</span>` : ""}</div>
    ...
    contentEl.innerHTML = `
      <div class="ticket">
        <div class="ticket-head">
          <div class="ticket-vehicle-block">
            ${v.foto_perfil ? `
      ...
      <div class="card">
        ...
        ${ultimo ? `
          <div class="timeline-title" style="margin-bottom:2px;">${esc(ultimo.tipo)}${ultimo.folio ? ` <span style="color:var(--green-bright);font-weight:400;font-size:12px;">· ${esc(t("folio_label"))} ${esc(ultimo.folio)}</span>` : ""}</div>
      ...
      <div class="card">
        ...
        <div class="actions-row">
          ${waHref ? `<a class="btn" href="${waHref}" rel="noopener">${esc(t("escribir_whatsapp"))}</a>` : ""}
