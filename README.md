...
- Lo mismo pasa al **iniciar un servicio nuevo** (que arranca en la etapa "Recibido") o si se regresa manualmente a esa etapa: aparece la misma ventana de WhatsApp, pero con un mensaje distinto avisando que el vehículo fue recibido, junto con el enlace directo a su ficha pública (`https://blaze151.github.io/taller-bassoco/?v=ID_DEL_VEHÍCULO`) para que el cliente pueda checar el avance cuando quiera.
- El **combustible** del vehículo ahora se elige de una lista fija (Gasolina, Diesel, Eléctrico, Gas) en vez de texto libre, para mantener los datos consistentes. Se traduce automáticamente en la ficha del cliente si la ve en inglés.
...
## 11.1 Ver fotos en grande (admin)

- El panel de administración ahora también tiene la función de "tocar para agrandar" en todas sus fotos (foto del vehículo, documentos, fotos por etapa del servicio en proceso, e historial) — antes solo existía en la ficha pública del cliente.
- Al tocar una foto se abre en grande con flechas para pasar a la siguiente/anterior si hay varias en el mismo grupo, y se puede cerrar con la ✕, tocando fuera de la foto, o con la tecla Escape.
- Tocar el botón "×" de quitar una foto ya no abre la vista ampliada por accidente.

## 11.3 Borrado real de archivos en GitHub

- Antes, al quitar una foto/documento (con el botón "×") o al borrar un vehículo completo, solo se quitaba la referencia en `vehicles.json` — el archivo en sí se quedaba abandonado en GitHub para siempre, ocupando espacio sin que nadie lo viera.
- Ahora, cuando guardas los cambios después de quitar una foto/documento, el archivo también se elimina de verdad del repositorio (no solo la referencia). Esto aplica en: foto de perfil, documentos del vehículo, fotos/documentos por etapa del servicio en proceso, al cancelar un servicio en proceso, y al quitar un servicio completo del historial.
- Al **borrar un vehículo completo**, ahora también se borra toda su carpeta `uploads/{id}/` en GitHub (antes se quedaba completa, huérfana — así se generaron las carpetas `uploads/1` y `uploads/2` que se tuvieron que limpiar a mano).
- Si por algún motivo el borrado de un archivo específico falla (por ejemplo, ya no existe), no se interrumpe el guardado — simplemente se ignora ese archivo y se sigue con los demás.

## 11.2 Número de unidad

- Nuevo campo en "Datos del vehículo" (junto a Placas): **Número de unidad**, para talleres que también identifican sus vehículos por número interno de flotilla, no solo por placas.
- Es opcional — si no se llena, simplemente no aparece.
- Se muestra en la ficha pública del cliente en una etiqueta verde junto a la placa, cada una en su propia línea para que no se rompan a la mitad en pantallas angostas.
- También aparece en el panel de administración: en la lista de vehículos y en las tarjetas del tablero por estado.
- Si el vehículo tiene número de unidad capturado, también aparece en los mensajes de WhatsApp de "recibido" y "listo para recoger" — útil para clientes con varias unidades del mismo modelo (ej. flotillas de reparto), para que sepan de cuál unidad se trata.

## 11.4 Autocompletar cliente al dar de alta un vehículo

- Al escribir el nombre o teléfono del cliente en un vehículo **nuevo**, el sistema sugiere clientes que ya se hayan capturado antes en otros vehículos (funciona con las sugerencias nativas del navegador al escribir).
- Si eliges o escribes exactamente un nombre/teléfono que ya existe, el otro campo se llena solo — así no hay que volver a escribir los datos de un cliente que ya tiene otro vehículo registrado (por ejemplo, una pastelería con varias unidades de reparto).
- Es solo una ayuda para no retipear: cada vehículo sigue guardando su propio nombre/teléfono de cliente por separado, no hay un directorio de clientes central todavía.
...
- **Las fotos e imágenes ahora se muestran directo desde el contenido del repositorio de GitHub (`raw.githubusercontent.com`), en vez de esperar a que se publique el sitio completo en GitHub Pages.** Esto es un cambio importante: antes, cada foto tenía que esperar a que GitHub Pages reconstruyera y publicara **todo el sitio** (lo cual podía tardar minutos, a veces varios, sobre todo con muchas fotos nuevas de golpe). Ahora cada foto se sirve directamente desde el repositorio en cuanto el commit se guarda — normalmente casi al instante. Como respaldo, el sistema sigue reintentando solo unas cuantas veces con pausas cortas (hasta ~1 minuto) por si esa ruta también tarda un poco, mostrando un pequeño círculo girando mientras lo hace.
- **Fotos en formato HEIC (iPhone) ahora se convierten solas a JPG.** Se encontró la causa real de varios reportes de "fotos que no cargan en Android/computadora pero sí en iPhone": algunas fotos de iPhone se guardan en formato HEIC, y aunque el sistema les pone extensión `.jpg`, el archivo por dentro seguía siendo HEIC — un formato que solo Safari/iOS sabe mostrar de forma nativa. Ahora, al detectar una foto HEIC, el sistema la convierte a JPG de verdad antes de subirla (usando la librería `heic2any`, incluida en `assets/heic2any.min.js`, que solo se carga cuando de verdad hace falta — no afecta la velocidad normal del sitio). Si por algún motivo la conversión falla, esa foto en particular se salta (no se sube rota) y se avisa claramente cuál fue.
