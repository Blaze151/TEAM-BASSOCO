...
(function () {
  ...
  const SESSION_KEY = "bassoco_admin_session";
  const DEFAULT_OWNER = "Blaze151";
  const DEFAULT_REPO = "TEAM-BASSOCO";
  ...
  const STR = {
    es: {
      ...
      repo_placeholder: "ej. TEAM-BASSOCO",
      branch_label: "Rama",
    ...
    en: {
      ...
      repo_placeholder: "e.g. TEAM-BASSOCO",
      branch_label: "Branch",
  ...
  const TRACK_BASE_URL = "https://blaze151.github.io/TEAM-BASSOCO/";
  ...
  try {
    const filtroGuardado = localStorage.getItem("bassoco_dash_filtro");
    const ordenGuardado = localStorage.getItem("bassoco_dash_orden");
  ...
  $("filter-select").addEventListener("change", (e) => {
    try { localStorage.setItem("bassoco_dash_filtro", e.target.value); } catch (err) {}
    renderDashboard($("search-box").value);
  });
  $("sort-select").addEventListener("change", (e) => {
    try { localStorage.setItem("bassoco_dash_orden", e.target.value); } catch (err) {}
    renderDashboard($("search-box").value);
  });
